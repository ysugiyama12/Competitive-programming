...
int main(){
    ...
    SegmentTree sg_even(N, v1), sg_odd(N, v2);
    rep(q,0,Q){
        if(a[q] == 1){
            if(b[q] % 2 == 0){
                ll val = sg_even.query_sum(b[q], b[q]+1);
                if(val >= c[q]){
                    sg_even.add_val(b[q], b[q]+1, -c[q]);
                }
            }else{
                ll val = sg_odd.query_sum(b[q], b[q]+1);
                if(val >= c[q]){
                    sg_odd.add_val(b[q], b[q]+1, -c[q]);
                }
            }
        }else if(a[q] == 2){
            ll min_val = sg_even.query_min(0, N);
            if(min_val >= b[q]){
                sg_even.add_val(0, N, -b[q]);
            }
        }else{
            ll min_val_odd = sg_odd.query_min(0, N);
            ll min_val_even = sg_even.query_min(0, N);
            if(min_val_odd >= b[q] && min_val_even >= b[q]){
                sg_odd.add_val(0, N, -b[q]);
                sg_even.add_val(0, N, -b[q]);
            }
        }
    }
    ...
    // print(ans);
    rep(i,0,N){
        ll val1 = sg_odd.query_sum(i, i+1);
        if(val1 <= 1e9) ans -= val1;
        ll val2 = sg_even.query_sum(i, i+1);
        if(val2 <= 1e9) ans -= val2;
        // print(val1, val2);
    }
    print(ans);
