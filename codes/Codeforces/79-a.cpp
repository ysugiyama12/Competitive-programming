...
int main(){
    ...
    rep(i,0,N){
        sg.update_val(i, i+1, T[i]);
    }
    rep(k,0,L){
        ll max_val = 0, idx = -1;
        rep(i,0,N-K+1){
            ll v = sg.query_sum(i, i+K);
            if(max_val < v){
                max_val = v;
                idx = i;
            }
        ...
        if(idx == -1) break;
        sg.update_val(idx, idx+K, 0);
    }
    ll ans1 = sg.query_sum(0,N);

    rep(i,0,N) sg.update_val(i,i+1, 1 - T[i]);
    rep(k,0,L){
        ll max_val = 0, idx = -1;
        rep(i,0,N-K+1){
            ll v = sg.query_sum(i, i+K);
