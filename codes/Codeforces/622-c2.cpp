...
ll left_sum[500010], right_sum[500010];
ll m[500010];
ll sum[500010] = {};
class SegmentTree {
  struct Node {
...
int main(){
    ...
    cin >> N;
    rep(i,0,N) cin >> m[i];
    SegmentTree sg(N+2, m);
    rep(i,0,N) sum[i+1] = sum[i] + m[i];
    if(N == 1){
        print(m[0]);
        return 0;
    }else if(N == 2){
    ...
    rep(i,0,N){
        sg.update_min(0, i+1, m[i]);
        left_sum[i] = sg.query_sum(0, i+1);
    }
    SegmentTree sg2(N+2, m);

    rrep(i,N-1,0){
        sg2.update_min(i+1, N, m[i]);
        right_sum[i] = sg2.query_sum(i, N);
    }
    ll ans = 0, idx = -1;
    rep(i,0,N){
        ll res = left_sum[i] + right_sum[i] - m[i];
        if(ans < res){
            ans = res;
            idx = i;
        }
