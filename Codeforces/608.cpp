...
int main(){
    ...
    rep(i,0,N){
        rep(j,i+1,N){
            needs[i] = max(needs[i], a[j] - (sum[j] - sum[i+1]));
        }
    }
    // printa(needs, N);
    // rep(i,0,N){
    //     print(needs[i] - )
    // }
    bool is_defense[5010] = {};
    worrior = K;
    rep(i,0,N){
        sort(tree[i].begin(), tree[i].end(), [&](ll l1, ll l2){
            return c[l1] > c[l2];
        });
    }
    // 0 0 1 2 2 5
    ...
    rep(i,0,N){
        worrior += b[i];
        diff[i] = worrior - needs[i];
    }
    SegmentTree sg(N, diff);
    vector<P> lp;
    // printa(diff, N);
    rep(i,0,N){
        for(auto &e: tree[i]){
            lp.push_back((P){i, e, c[e]});
            // print(i,e, c[e]);
        }
    }
    ...
    for(auto &e: lp){
        if(is_defense[e.to]) continue;
        ll min_val = sg.query_min(e.from, N);
        // print(e.from, e.to, min_val);

        if(min_val > 0){
            sg.add_val(e.from, N, -1);

            is_defense[e.to] = true;
        }
    }
    // printa(diff, N);
    // rrep(i,N-1,0){
